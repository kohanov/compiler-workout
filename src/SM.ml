...
let rec eval envr cfg programs =
    let (ctrl, stack, (st, input, output)) = cfg in match programs with
    | []               -> cfg
    | BINOP op    :: o -> let l :: r :: rest = stack in 
                          let value = Expr.eval st (Expr.Binop (op, Expr.Const r, Expr.Const l)) in
                          eval envr (ctrl, value :: rest, (st, input, output)) o
    | CONST c     :: o -> eval envr (ctrl, c :: stack, (st, input, output)) o
    | READ        :: o -> eval envr (ctrl, hd input :: stack, (st, tl input, output)) o
    | WRITE       :: o -> eval envr (ctrl, tl stack, (st, input, output @ [hd stack])) o
    | LD s        :: o -> eval envr (ctrl, (State.eval st s) :: stack, (st, input, output)) o
